...
double sineOscillator(double time, double freq, double amp) {
	double s = sin(time*freq*3.141592*2.0) * amp;
...
EZ_sample sampleClbk(double time, int channel) {
  ...
  double sf = sineOscillator(time, 440.0, 0.2);
...
int main (int argc, char **argv) {
    ...
    EZ_sfx_init(CD41KHZ, 2, 16, 512, &playRaw);
